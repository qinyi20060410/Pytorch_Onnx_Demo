...
#define CHECK(status)                                         \
  do {                                                        \
    auto ret = (status);                                      \
    if (ret != 0) {                                           \
      Trt::gLogError << "Cuda failure: " << ret << std::endl; \
      abort();                                                \
    }                                                         \
  } while (0)
...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                             \
  do {                                                                      \
    if (!(status)) {                                                        \
      Trt::gLogError << errMsg << " Error in " << __FILE__ << ", function " \
...
std::vector<size_t> topKMagnitudes(const std::vector<T>& v, const size_t k) {
  std::vector<size_t> indices =
      TrtCommon::argMagnitudeSort(v.cbegin(), v.cend());
  indices.resize(k);
  return indices;
}
...
inline void setDummyInt8Scales(const IBuilderConfig* c, INetworkDefinition* n) {
  ...
  if (c->getFlag(BuilderFlag::kINT8)) {
    Trt::gLogWarning << "Int8 calibrator not provided. Generating dummy per "
                        "tensor scales. Int8 accuracy is not guaranteed."
                     << std::endl;
...
inline void loadLibrary(const std::string& path) {
  ...
  if (handle == nullptr) {
#ifdef _MSC_VER
    Trt::gLogError << "Could not load plugin library: " << path << std::endl;
#else
    Trt::gLogError << "Could not load plugin library: " << path
