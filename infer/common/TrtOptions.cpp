...
template <typename T>
bool checkEraseOption(Arguments& arguments, const std::string& option,
                      T& value) {
...
// Like checkEraseOption, but sets value to false if arguments contain the
// option. This function should be used for options that default to true.
bool checkEraseNegativeOption(Arguments& arguments, const std::string& option,
                              bool& value) {
  bool dummy;
...
template <typename T>
bool checkEraseRepeatedOption(Arguments& arguments, const std::string& option,
                              std::vector<T>& values) {
...
void processShapes(std::unordered_map<std::string, ShapeRange>& shapes,
  ...
  if (((minShapes || maxShapes) &&
       !optShapes)  // minShapes only, maxShapes only, both minShapes and
                    // maxShapes
...
void BuildOptions::parse(Arguments& arguments) {
  ...
  if (nvtxModeString == "default") {
    ...
    throw std::invalid_argument(std::string("Unknown nvtxMode: ") +
                                nvtxModeString);
  }
...
void AllOptions::parse(Arguments& arguments) {
  ...
  if (build.maxBatch && inference.batch) {
    ...
    // given and inference batch is greater than maxBatch, use inference batch
    // also for maxBatch
