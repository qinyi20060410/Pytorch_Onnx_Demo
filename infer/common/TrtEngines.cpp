  ...
  switch (model.baseModel.format) {
    ...
    case ModelFormat::kONNX: {
      ...
      parser.onnxParser.reset(
          createParser(network, Trt::gLogger.getTRTLogger()));
...
class RndInt8Calibrator : public nvinfer1::IInt8EntropyCalibrator2 {
 public:
  RndInt8Calibrator(int batches, std::vector<int>& elemCount,
...
RndInt8Calibrator::RndInt8Calibrator(int batches, std::vector<int>& elemCount,
                                     const std::string& cacheFile,
...
bool setTensorScales(const INetworkDefinition& network, float inScales = 2.0f,
...
ICudaEngine* networkToEngine(const BuildOptions& build,
    ...
    if (profile) {
      ...
      if (isDynamicInput) {
        ...
        if (shape == build.shapes.end()) {
          ...
          Trt::gLogWarning << "Dynamic dimensions required for input: "
                           << input->getName()
  ...
  if (profile && hasDynamicShapes) {
    SMP_RETVAL_IF_FALSE(profile->isValid(),
                        "Required optimization profile is invalid", nullptr,
                        err);
    SMP_RETVAL_IF_FALSE(config->addOptimizationProfile(profile) != -1,
                        "Error in add optimization profile", nullptr, err);
  }
  ...
  if (build.int8 && !build.fp16) {
    Trt::gLogInfo << "FP32 and INT8 precisions have been specified - more "
  ...
  if ((build.int8 && build.calibration.empty()) || int8IO) {
    ...
    // tensors use int8, because auto calibration does not support this case.
    SMP_RETVAL_IF_FALSE(setTensorScales(network), "Error in set tensor scales.",
                        nullptr, err);
  } else if (build.int8) {
    ...
    if (!build.shapesCalib.empty()) {
      ...
      SMP_RETVAL_IF_FALSE(profileCalib->isValid(),
                          "Calibration profile is invalid", nullptr, err);
      SMP_RETVAL_IF_FALSE(config->setCalibrationProfile(profileCalib),
                          "Error in set calibration profile", nullptr, err);
    }
...
ICudaEngine* modelToEngine(const ModelOptions& model, const BuildOptions& build,
  ...
  TrtUniquePtr<IBuilder> builder{
      createInferBuilder(Trt::gLogger.getTRTLogger())};
  if (builder == nullptr) {
...
void dumpRefittable(nvinfer1::ICudaEngine& engine) {
  TrtUniquePtr<IRefitter> refitter{
      createInferRefitter(engine, Trt::gLogger.getTRTLogger())};
  ...
  for (int i = 0; i < nbAll; ++i) {
    Trt::gLogInfo << layerNames[i] << " " << weightsRoles[i] << std::endl;
  }
}
...
ICudaEngine* loadEngine(const std::string& engine, int DLACore,
  ...
  TrtUniquePtr<IRuntime> runtime{
      createInferRuntime(Trt::gLogger.getTRTLogger())};
  if (DLACore != -1) {
